(() => {
  type ToolSection = HTMLElement & { dataset: DOMStringMap };
  type UnitConfig = {
    name: string;
  ...
  function initCalculator(): void {
    ...
    const grid = getById<HTMLElement>("calc-grid");
    const clearBtn = getById<HTMLButtonElement>("calc-clear");
    const equalsBtn = getById<HTMLButtonElement>("calc-equals");
  ...
  function initConverter(): void {
    ...
    const categoryEl = getById<HTMLSelectElement>("conv-category");
    const valueEl = getById<HTMLInputElement>("conv-value");
    const fromEl = getById<HTMLSelectElement>("conv-from");
    const toEl = getById<HTMLSelectElement>("conv-to");
    const runBtn = getById<HTMLButtonElement>("conv-run");
    const resultEl = getById<HTMLOutputElement>("conv-result");
    ...
    function populateUnits(): void {
      ...
      Object.entries(units).forEach(([unitKey, config]) => {
        ...
        const toOption = fromOption.cloneNode(true) as HTMLOptionElement;
    ...
    function convert(): void {
      ...
      resultOutput.textContent =
        value + " " + fromSelect.value + " = " + converted.toFixed(6) + " " + toSelect.value;
    }
  ...
  function initRatioCalculator(): void {
    const leftEl = getById<HTMLInputElement>("ratio-left");
    const rightEl = getById<HTMLInputElement>("ratio-right");
    const knownLeftEl = getById<HTMLInputElement>("ratio-known-left");
    const knownRightEl = getById<HTMLInputElement>("ratio-known-right");
    const calcBtn = getById<HTMLButtonElement>("ratio-calc");
    const clearBtn = getById<HTMLButtonElement>("ratio-clear");
    const resultEl = getById<HTMLOutputElement>("ratio-result");
    ...
    function simplifyRatio(left: number, right: number): [number, number] {
  ...
  function initOhmsTool(): void {
    const voltage = getById<HTMLInputElement>("ohm-voltage");
    const current = getById<HTMLInputElement>("ohm-current");
    const resistance = getById<HTMLInputElement>("ohm-resistance");
    const power = getById<HTMLInputElement>("ohm-power");
    const calcBtn = getById<HTMLButtonElement>("ohm-calc");
    const clearBtn = getById<HTMLButtonElement>("ohm-clear");
    const result = getById<HTMLOutputElement>("ohm-result");
  ...
  function initVoltageCalculator(): void {
    const currentEl = getById<HTMLInputElement>("volt-current");
    const resistanceEl = getById<HTMLInputElement>("volt-resistance");
    const powerEl = getById<HTMLInputElement>("volt-power");
    const calcBtn = getById<HTMLButtonElement>("volt-calc");
    const clearBtn = getById<HTMLButtonElement>("volt-clear");
    const resultEl = getById<HTMLOutputElement>("volt-result");
  ...
  function initResistorCalculator(): void {
    const vinEl = getById<HTMLInputElement>("res-vin");
    const voutEl = getById<HTMLInputElement>("res-vout");
    const currentEl = getById<HTMLInputElement>("res-current");
    const calcBtn = getById<HTMLButtonElement>("res-calc");

    const resultWrap = getById<HTMLElement>("res-result");
    const rLine = getById<HTMLParagraphElement>("res-r-line");
    const pLine = getById<HTMLParagraphElement>("res-p-line");
    const noteLine = getById<HTMLParagraphElement>("res-note-line");
  ...
  function initBatteryCalculator(): void {
    const voltageEl = getById<HTMLInputElement>("bat-voltage");
    const currentEl = getById<HTMLInputElement>("bat-current");
    const countEl = getById<HTMLInputElement>("bat-count");
    const calcBtn = getById<HTMLButtonElement>("bat-calc");
    const clearBtn = getById<HTMLButtonElement>("bat-clear");

    const resultWrap = getById<HTMLElement>("bat-result");
    const seriesLine = getById<HTMLParagraphElement>("bat-series-line");
    const parallelLine = getById<HTMLParagraphElement>("bat-parallel-line");
    const noteLine = getById<HTMLParagraphElement>("bat-note-line");
